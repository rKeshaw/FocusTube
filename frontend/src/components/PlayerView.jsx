...
export default function PlayerView({ video, onBack }) {
  ...
  // Check backend availability and fetch streams on mount
  useEffect(() => {
    const init = async () => {
      setLoading(true);
      setError(null);
      ...
      const [streamData, healthy] = await Promise.all([
        getVideoStreams(video.videoId).catch((e) => { setError(e.message); return null; }),
        checkBackendHealth(),
      ]);
  ...
  // Initialise player once streams are ready
  useEffect(() => {
    if (!streams || !videoRef.current) return;

    const resolved = resolveStreamUrl(streams);
    if (!resolved) {
      setError('No playable stream found for this video.');
      return;
    }
    ...
    <div className="player-root animate-fade-up">
      ...
      <div className="player-info">
        ...
        <div className="player-actions">
          ...
          >
            {inWL ? (
              <svg viewBox="0 0 24 24" fill="currentColor"><path d="M5 3a2 2 0 0 0-2 2v16l7-3 7 3V5a2 2 0 0 0-2-2H5Z"/></svg>
            ) : (
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          {/* Download — only if backend is up */}
          {backendAvailable && (
